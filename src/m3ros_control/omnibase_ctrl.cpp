...
OmnibaseCtrl::OmnibaseCtrl(m3::M3Omnibase* obase_shr_ptr, m3::M3OmnibaseShm* obase_shm_shr_ptr, std::string nodename, BASE_CTRL_MODE mode = SDS):
	obase_shr_ptr_(NULL), obase_shm_shr_ptr_(NULL), node_name(nodename), ctrl_mode(mode) {
	...
	if(((M3OmnibaseStatus*) obase_shr_ptr_->GetStatus())->calibrated()) {
		...
		m3rt::M3_INFO("Omnibase is not calibrated. To use the omnibase, calibrate it.\n");
	}
...
void OmnibaseCtrl::init_sds() {
	...
	odom_publisher_g = ros_nh_ptr_->advertise<nav_msgs::Odometry>(
			"odom", 1, true);
