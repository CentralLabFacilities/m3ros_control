...
RosControlComponent::RosControlComponent() :
        m3rt::M3Component(MAX_PRIORITY), state_mutex_(NULL), spinner_running_(
				...
				NULL), zlift_shr_ptr_(NULL), obase_shr_ptr_(NULL), pwr_shr_ptr_(NULL),
...
bool RosControlComponent::RosInit() {
    ...
    if (ros::master::check()) {
        ...
        obase_ptr_ = new OmnibaseCtrl(obase_shr_ptr_, obase_shm_shr_ptr_, ros_node_name + "_base_controller");
