    ...
    if (req.command.state.size() > 0
        ...
        if (!was_estop_) {
            for (size_t i = 0; i < req.command.group_name.size(); i++) {
                ...
                int ret_tmp = hw_ptr_->changeState(req.command.state[i],
...
bool RosControlComponent::RosInit(m3::M3Humanoid* bot, m3::M3JointZLift* lift) {
    ...
    if (ros::master::check()) {
        ...
        // Create a rt thread for omnibase control via sds
...
void RosControlComponent::RosShutdown() {
    ...
    if(hst) {
        ...
        usleep(1250000);
        if (sys_thread_active)
        m3rt::M3_ERR("Real-time thread did not shutdown correctly\n");
        //rt_task_delete(task);
        rt_shm_free(nam2num(MEKA_ODOM_SHM));
    }
...
void *ros_async_spinner(void * arg) {
    ...
    RT_TASK *task;
    task = rt_task_init_schmod(nam2num(ROS_ASYNC_SP), prio, 0, 0, SCHED_FIFO, 0xFF);
    if (task == NULL) {
...
void *rosmain_async_spinner(void * arg) {
    ...
    RT_TASK *task;
    task = rt_task_init_schmod(nam2num(ROS_ASYNC_MSP), prio, 0, 0, SCHED_FIFO, 0xFF);
    if (task == NULL) {
...
void* rt_system_thread(void * arg) {
    ...
    task = rt_task_init_schmod(nam2num(MEKA_OMNIBASE_SHM), 3, 0, 0, SCHED_FIFO, 0xF);
